...
import com.rp.autoClick.Skillsoft.Courses.Type2.Type2Course;
...
public class MainRunner {
	...
	public static void main( final String[] args ) throws FindFailed,
		...
		// Set DensityPoint Reference to 3200 x 1800
		DP.setGlobalReferenceResolution( 3200, 1800 );

		App.focusedWindow( ).click(
				DP.getPtrn( MainRunner.windowButtons ).targetOffset(
						DP.getLoc( -10, 0 ) ) );
		s.wait( 1f );
		...
		.find( DP.getPtrn( MainRunner.tableContentsButton.similar( 0.5f ) ) )
				.highlight( 1 ).offset( DP.getLoc( -45, -20 ) )
				.setSize( DP.getX( 1790 ), DP.getY( 1045 ) );

		final Type2Course course = new Type2Course( f, r );

		s.wait( 3f );

		course.CourseTest.runStragity( );
		...
		if ( App.focus( "Army eLearning" ) == null ) {
			...
			s.click( );
			s.click( s.exists( MainRunner.browser_addressBar, 120 ).offset(
					100, 0 ) );
		...
		if ( exist != null ) {
			...
			for ( final Match c : courses ) {
				...
				s.hover( c );
				// hover over.
				if ( ( ( exist = c.grow( 0, s.getW( ) - c.getX( ) - c.getW( ),
						0, 0 ).exists( MainRunner.course_launch, 5 ) ) != null )
	...
	public static boolean olderStyleModules( Region r ) throws FindFailed {
		...
		while ( ( clickEnd = r.exists( MainRunner.continue_course, 0 ) ) == null ) {
			...
			for ( int i = 0; i < 10; i++ ) {
				clickEnd = r
						.exists( MainRunner.arrow_right.similar( 0.78f ), 5 );
	...
	public static boolean olderStyleTestQuestion( final Region r,
		...
		while ( null != question.initialAttempt( r, 2 ) )
			if ( ( System.currentTimeMillis( ) - time ) > 120000 ) {
